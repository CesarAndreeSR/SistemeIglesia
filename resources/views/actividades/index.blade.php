                    ...
                    @foreach($actividades as $actividad)
                        <tr class="hover:bg-[#FAF8F5] transition-colors duration-200">
                            <td class="px-4 md:px-6 py-4">
                                <div class="text-sm font-medium text-[#1F2937]">{{ $actividad->titulo }}</div>
                            </td>
                            ...
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-medium rounded-full {{ $actividad->estado === 'pendiente' ? 'bg-[#E9D8B6] text-[#A97142]' : ($actividad->estado === 'en_proceso' ? 'bg-[#C8A26E]/20 text-[#A97142]' : 'bg-[#E9D8B6] text-[#A97142]') }}">
                            ...
                            <td class="px-4 md:px-6 py-4 whitespace-nowrap text-right text-sm font-medium relative">
                                <div class="relative inline-block text-left">
                                    <button onclick="toggleDropdown('dropdown-{{ $actividad->id }}')" class="p-2 rounded-full hover:bg-[#FAF8F5] transition-colors focus:outline-none">
                                        <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"></path>
                                        </svg>
                                    </button>

                                    <div id="dropdown-{{ $actividad->id }}" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-[#E9D8B6] z-50">
                                        <div class="py-1">
                                            <a href="{{ route('actividades.show', ['actividad' => $actividad->id]) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-[#FAF8F5] hover:text-[#C8A26E] transition-colors">
                                            ...
                                            <a href="{{ route('actividades.edit', ['actividad' => $actividad->id]) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-[#FAF8F5] hover:text-[#A97142] transition-colors">
                                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
