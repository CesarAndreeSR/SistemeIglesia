...
<div class="space-y-8 animate-fade-in">
    ...
    <div class="bg-gradient-to-r from-teal-600 to-teal-700 rounded-2xl shadow-lg p-8">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('users.index') }}" class="bg-white/20 hover:bg-white/30 text-white px-6 py-3 rounded-xl backdrop-blur-sm transition-all duration-200 flex items-center space-x-2">
        ...
        <form action="{{ route('users.update', $user) }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required placeholder="Ej: Juan Pérez"
                ...
                <div>
                    ...
                    <input type="text" name="username" id="username" value="{{ old('username', $user->username) }}" required placeholder="Ej: juanperez"
                        class="mt-1 block w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-teal-500 focus:ring-4 focus:ring-teal-100 transition-all duration-200 text-gray-900 placeholder-gray-400">
                    @error('username')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" placeholder="Ej: juan@ejemplo.com"
                        class="mt-1 block w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-teal-500 focus:ring-4 focus:ring-teal-100 transition-all duration-200 text-gray-900 placeholder-gray-400">
                    @error('email')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div>
                    ...
                    <input type="text" name="telefono" id="telefono" value="{{ old('telefono', $user->telefono) }}" placeholder="Ej: +51 999 999 999"
                        class="mt-1 block w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-teal-500 focus:ring-4 focus:ring-teal-100 transition-all duration-200 text-gray-900 placeholder-gray-400">
                    @error('telefono')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div>
                    ...
                    <select name="cargo_id" id="cargo_id"
                        ...
                        @foreach($cargos as $cargo)
                            <option value="{{ $cargo->id }}" {{ old('cargo_id', $user->cargo_id) == $cargo->id ? 'selected' : '' }}>{{ $cargo->nombre }}</option>
                        @endforeach
                    </select>
                    @error('cargo_id')
                        <p class="mt-2 text-sm text-red-600 flex items-center">
                ...
                <div class="flex items-center p-4 bg-gray-50 rounded-xl">
                    <input type="checkbox" name="estado" id="estado" value="1" {{ $user->estado ? 'checked' : '' }}
                ...
                <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                    <a href="{{ route('users.index') }}" class="px-6 py-3 border-2 border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 transition-all duration-200 flex items-center space-x-2 font-medium">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
